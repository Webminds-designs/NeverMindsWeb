...
const ProfileDashboard = () => {
  ...
  const { profileData, isLoading, error, updateProfile } = useUserProfile(user?.id);
      ...
      <main className="flex flex-col lg:flex-row w-full p-4 md:p-6 lg:p-8">
        ...
        <div className="w-full lg:w-4/12 flex flex-col mt-10 lg:mt-0 lg:ml-8">
          ...
          <div className="flex flex-col items-center justify-center">
            {isLoading ? (
              <div className="animate-pulse bg-gray-200 rounded-3xl w-full h-96"></div>
            ) : profileData ? (
              <ProfileCard
                ...
                phone={
                  profileData.role === "student" 
                    ? profileData.studentDetails?.guardianContact || "Not provided" 
                    : profileData.teacherDetails?.contactNo || "Not provided"
                }
                address={profileData.address || "Not provided"}
                profileImage={profileImage} // You might want to use a profile image from the API
                greeting={getGreeting()}
                onEdit={handleEdit}
                progress={85} // You might want to calculate this based on quiz scores
